...
public class Package {
    ...
    public Package(Repository repo,String name){
        try {
            ...
            Commons.log.log(Level.WARNING, "[NexT][Package] URL malformed.", ex);
        }
    }
    ...
    public boolean downloadPackage(){
        ...
        try {
            Commons.log.info("[NexT][Package] GET "+info.get("name")+" << "+new URL(url, info.get("name")+"/pack.zip"));
            ...
            Commons.log.log(Level.SEVERE, "[NexT][Package] URL malformed.", ex);
            return false;
        }
        return true;
    }
    ...
    public boolean installPackage(File location){
        ...
        try {
            Commons.log.info("[NexT][Package] PUT "+info.get("name")+" >> "+location.getAbsolutePath());
            return Toolkit.unzipFile(tmpfile, location);
        } catch (IOException ex) {
            Commons.log.log(Level.SEVERE, "[NexT][Package] IO Exception while unzipping.", ex);
            return false;
        }
    }
